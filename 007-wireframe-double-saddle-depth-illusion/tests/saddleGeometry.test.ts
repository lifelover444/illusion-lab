...
describe('wireframe saddle geometry', () => {
  ...
  it('stacks two synchronized saddle models with the upper model in a visible inverted phase', () => {
    expect(config.composition.modelInstances).toHaveLength(2);

    const [lower, upper] = config.composition.modelInstances;
    expect(lower.name).toBe('lower-saddle');
    expect(upper.name).toBe('upper-inverted-saddle');
    expect(lower.rotationXDegrees).toBe(0);
    expect(upper.rotationXDegrees).toBe(0);
    expect(lower.rotationZDegrees).toBe(0);
    expect(upper.rotationZDegrees).toBe(90);
    expect(lower.zOffset).toBeLessThan(0);
    expect(upper.zOffset).toBeGreaterThan(0);
    expect(Math.abs(lower.zOffset)).toBeCloseTo(upper.zOffset, 5);
    expect(config.composition.sharedRotationRig).toBe(true);
    expect(config.profiles.preview.seconds / config.style.rotationCycleSeconds).toBe(2);
  });
});
