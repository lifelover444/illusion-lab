...
def create_saddle_particles(geometry, style, collection, parent):
...
def animate_scene(rotation_rig, surface_rig, pulse_rig, pulse_material, profile, style):
    scene = bpy.context.scene
...
def main():
    ...
    blend_path = project_root / "scene" / "wireframe-saddle-depth-illusion.blend"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    blend_path.parent.mkdir(parents=True, exist_ok=True)

    clear_scene()
    collection = make_collection("wireframe-saddle-depth-illusion")
    rotation_rig = create_rig("saddle-rotation-rig")
    surface_rig = create_rig("saddle-surface-rig", rotation_rig)

    create_saddle_wireframe(geometry, style, collection, surface_rig)
    create_saddle_particles(geometry, style, collection, surface_rig)
    ...
    animate_scene(rotation_rig, surface_rig, pulse_rig, pulse_material, profile, style)
