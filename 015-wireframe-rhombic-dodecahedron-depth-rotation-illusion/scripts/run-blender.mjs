...
const config = JSON.parse(
  readFileSync(path.join(__dirname, 'rhombic-dodecahedron-config.json'), 'utf8')
);
...
const commandMap = {
  ...
  hd: { profile: 'hd', flag: '--render' },
  hd60: { profile: 'hd60', flag: '--render' }
};

if (!Object.hasOwn(commandMap, command)) {
  console.error(`Unknown command "${command}". Use one of: ${Object.keys(commandMap).join(', ')}.`);
  process.exit(1);
}
...
function decodedFrameHash(videoPath, frameNumber) {
  ...
  if (hash.status !== 0) throw new Error(hash.stderr || `ffmpeg frame ${frameNumber} hash failed.`);
  const dataLine = hash.stdout
    .split(/\r?\n/)
...
function verifyVideo(profileName) {
  ...
  const turns = profile.seconds / config.style.rotationCycleSeconds;
  ...
  const repeatedTailFrame = firstFrameHash === lastFrameHash;
  if (repeatedTailFrame) throw new Error('The encoded tail repeats the first frame.');
  ...
  const inspection = {
    ...
    highResolutionRenderAttempted: profileName !== 'preview'
  };
  writeFileSync(
  ...
  const sheetPath = path.join(
    ...
    'output',
    profileName === 'preview'
      ? 'rhombic-dodecahedron-motion-contact-sheet.png'
      : `rhombic-dodecahedron-motion-contact-sheet-${profileName}.png`
  );
  ...
  console.log(
    `Verified ${profileName}: H.264, ${actualFrames} frames, ${profile.seconds}s, ${actualFps}fps at ${stream.width}x${stream.height}; repeated tail: ${repeatedTailFrame}.`
  );
